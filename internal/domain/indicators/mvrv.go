...
func MVRV(mvrvRatios []float64) []float64 {
	...
	for i, ratio := range mvrvRatios {
		...
		case ratio >= 5.0:
			score = 0
		case ratio <= 1.0:
			// Piecewise linear mapping: 0.5->100, 1.0->75
			score = linearInterp(ratio, 0.5, 1.0, 100, 75)
		case ratio <= 2.0:
			// 1.0->75, 2.0->40
			score = linearInterp(ratio, 1.0, 2.0, 75, 40)
		case ratio <= 3.5:
			// 2.0->40, 3.5->10
			score = linearInterp(ratio, 2.0, 3.5, 40, 10)
		default:
			// 3.5->10, 5.0->0
			score = linearInterp(ratio, 3.5, 5.0, 10, 0)
		}
